...
const LoginPage: React.FC = () => {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (view === 'signup') {
        ...
        alert("Account created! Please check your email to verify (if email verification is enabled in your project). You can now try logging in.");
        setView('login');
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAF8] flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="relative bg-white/90 backdrop-blur-xl p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-white max-w-md w-full animate-slide-up z-10">
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-6 py-4 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-primary/20 outline-none transition-all font-bold text-sm pr-14"
                required
              />
              <button
                type="button"
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full py-5 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-[0.25em] shadow-xl hover:bg-secondary transition-all active:scale-[0.98] disabled:bg-gray-200 mt-4"
          >
      ...
      <p className="mt-8 text-[9px] font-black text-gray-400 uppercase tracking-widest opacity-40">
